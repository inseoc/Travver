...
7. **[필수] 장소 중복 금지**: 전체 여행 기간 동안 동일한 장소가 두 번 이상 등장하면 안 됩니다. 각 Day의 일정은 서로 완전히 다른 고유한 장소로만 구성하세요.
8. **[필수] 이동 동선 최적화**:
   - 하루 일정은 지리적으로 가까운 장소들끼리 묶어서 구성하세요
   - 첫 번째 장소에서 시작해 순서대로 이동할 때, 왔다갔다하지 않고 자연스럽게 이어지도록 배치하세요
   - 장소 간 이동거리가 5km를 넘지 않도록 주의하세요 (대중교통 30분 이내 권장)
   - 도시 반대편으로 갑자기 이동하는 일정은 피하세요
...
## 장소명 규칙 (매우 중요!)
- **반드시 실제 존재하는 구체적인 장소명을 사용해야 합니다**
- "포토스팟", "맛집", "관광지" 같은 일반적인 명칭은 절대 사용하지 마세요
- 올바른 예시: "이치란 라멘 도톤보리점", "오사카성 천수각", "도톤보리 글리코상"
- 잘못된 예시: "오사카 맛집 추천", "포토스팟 1", "관광지 방문"
- 수집된 장소 정보에 있는 실제 장소명을 우선 사용하세요
- 좌표는 반드시 해당 장소의 실제 위도/경도를 사용하세요
...
5. 식사 시간 포함 (아침, 점심, 저녁)
6. 각 장소의 실제 좌표 포함
7. **[필수] 모든 장소명은 반드시 실제 존재하는 구체적인 이름 사용** (예: "이치란 라멘 도톤보리점", "오사카성 천수각")
   - "맛집", "포토스팟", "관광지" 같은 일반 명칭 절대 금지
8. **[필수] 장소 중복 금지**: 전체 {num_days}일 동안 동일한 장소가 두 번 이상 나오면 안 됩니다
9. **[필수] 이동 동선 최적화**:
   - 하루 일정은 지리적으로 인접한 장소들로 구성 (같은 지역/구역 중심)
   - 장소 간 이동거리 5km 이내, 대중교통 30분 이내로 유지
   - 도시 반대편으로 갑자기 이동하는 비효율적 동선 금지
   - 첫 장소부터 마지막 장소까지 자연스럽게 이어지는 경로로 배치

다음 JSON 형식으로 응답해주세요 (place에는 반드시 실제 장소명 사용!):
{{
  "daily_plans": [
    {{
      "day": 1,
      "date": "{start_date}",
      "theme": "첫날 테마 (예: 도톤보리 & 난바 탐방)",
      "schedules": [
        {{
          "order": 1,
          "time": "10:00",
          "place": "실제 장소명 (예: 오사카성 천수각, 이치란 라멘 도톤보리점)",
